...
def sequence_graph(df):  # df:seqdf -> graph: sequencesfgs
    ...
    df = df.with_columns(
        pl.when(pl.col("Sequence").str.slice(-1) == "C").then(pl.lit("#32CD32"))  # canestri = verde lime
        .when(pl.col("Sequence").str.slice(-1) == "E").then(pl.lit("red"))  # errori = rosso
    ...
    chart = alt.Chart(df_filtered.to_pandas()).mark_circle(size=50).encode(
        ...
        color=alt.Color("Color:N", 
            legend=alt.Legend(
                title="After a",  # il colore rappresenta l'esito del tiro precedente -> "dopo un"
                orient="right",  # per evitare che vada in mezzo al grafico
                labelExpr="datum.label === '#32CD32' ? 'make' : datum.label === 'red' ? 'miss' : '1st shot'"
            ),
            scale=alt.Scale(domain=["#32CD32", "red", "white"], range=["#32CD32", "red", "white"]) #32CD32=verde lime, contrasto col tema scuro
        ),
        tooltip=["Sequence", "FG%"]
    ).properties(
...
def singlegame_fg_graph(modified_events):  # list:events -> graph:shotsgraph
    ...
    for _, event in enumerate(modified_events):
        if event == "C":
            makes += 1
            color = "#32CD32"  # canestri = verde lime
        elif event == "E":
            color = "red"  # errori = rosso
        total_shots += 1
    ...
    graph = alt.Chart(df).mark_circle(size=50).encode(
        ...
        color=alt.Color(  # imposto i colori
            ...
            legend=alt.Legend(
                ...
                orient="right",
                labelExpr="datum.label === '#32CD32' ? 'make' : 'miss'"  # verde lime associato ai make, il rosso ai miss
            )
        ),
...
def gameforgame_stats_graph(player_name):  # str:name -> graph:sumupgraph
    ...
    chart = alt.Chart(long_data).mark_line(point=True, strokeWidth=1).encode(
        ...
        color=alt.Color(
            "Metric:N", 
            scale=alt.Scale(domain=["Makes", "Misses", "Streaks"],
                            range=["#32CD32", "red", "yellow"]),  # canestri=verde lime, errori=rosso, streaks=giallo
...
def top5_graph(data): #list:data -> graph:top5graph
    df = pd.DataFrame(data, columns=['Player', 'Makes', 'Misses', 'Streaks'])
    ...
    color_scale = alt.Scale(
        domain=['Makes', 'Misses', 'Streaks'],
        range=['#32CD32', 'red', 'yellow']  # verde lime, rosso, giallo
    )
    
    chart = alt.Chart(df_long).mark_bar().encode(
        x=alt.X('Player:N', title='Player', axis=alt.Axis(labelAngle=0)),  
